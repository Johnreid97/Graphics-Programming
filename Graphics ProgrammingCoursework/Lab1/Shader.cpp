...
void Shader::initialise(const std::string& fileVert, const std::string& fileFrag, const std::string& fileGeom)
{
	program = glCreateProgram(); // create shader program (openGL saves as ref number)
	shaders[0] = CreateShader(LoadShader(fileVert), GL_VERTEX_SHADER); // create vertex shader
	shaders[1] = CreateShader(LoadShader(fileFrag), GL_FRAGMENT_SHADER); // create fragment shader
	shaders[2] = CreateShader(LoadShader(fileGeom), GL_GEOMETRY_SHADER); //Create geometry shader
