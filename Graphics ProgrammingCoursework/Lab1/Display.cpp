...
Display::~Display()
{
	...
	SDL_DestroyWindow(sdlWindow); // detete window (make sure to delete the context and the window in the opposite order of creation in initialiseDisplay())
	SDL_Quit();
}
...
void Display::initialiseDisplay()
{
	...
	glEnable(GL_CULL_FACE); //dont create faces that are not pointing at the camera
